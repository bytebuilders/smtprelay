...
SRC_PKGS := cmd # directories which hold app source excluding tests (not vendored)
...
uninstall:
	@cd ../installer; \
	helm uninstall smtprelay --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: ci
ci: build unit-tests #lint check-license cover verify
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/smtprelay run \
		--v=3
